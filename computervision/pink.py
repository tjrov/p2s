...
from cv2 import Canny, HoughLines, imshow, waitKey, destroyAllWindows, \
    resize, COLOR_BGR2HSV, imread, cvtColor, inRange, bitwise_and, \
    line, LINE_AA
...
def contains_similar_line(image, line_list, new_line, epsilon_angle=0.1, epsilon_shift=5):
    # Ax + By = C
    # new_line = (A, B, C)
...
def lines_are_close(image, line1, line2, epsilon_shift):
    ...
    perpendicular_line = line_from_point_and_slope((width/2, height/2), perpendicular_slope)
...
def fit_perpendicular_axes(angles_list, iterations=20, d_theta=pi/6):
    ...
    for _ in range(iterations):
        error1 = calculate_angular_mse(current_angle + d_theta, perpendicular_axis + d_theta, angles_list)
        error2 = calculate_angular_mse(current_angle - d_theta, perpendicular_axis - d_theta, angles_list)
        if error1 < error2:
...
if __name__ == '__main__':
    ...
    target_hsv = [int(hsv_unscaled[0]*255), int(hsv_unscaled[1]*255), hsv_unscaled[2]]

    thresh = 100
    lower_pink = array([target_hsv[0] - thresh, target_hsv[1] - thresh, target_hsv[2] - thresh])  # in HSV
    upper_pink = array([target_hsv[0] + thresh, target_hsv[1] + thresh, target_hsv[2] + thresh])  # in HSV
    ...
    for i in range(a):
        ...
        if not contains_similar_line(img, unique_lines, line_equation, epsilon_angle=0.15, epsilon_shift=20):
    ...
    averaged_cluster = {average_angles(line_cluster[axis]): line_cluster[axis] for axis in line_cluster}
